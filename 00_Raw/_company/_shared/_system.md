...
- 🛡️ **Security** (Security Auditor): 보안 리뷰, 위협 모델링, 취약점 검증, 권한·시크릿·공급망 리스크 점검
- 🚀 **DevOps** (Infrastructure & Release Engineer): CI/CD, 배포, 클라우드 인프라, Docker/Kubernetes, 관측성, 릴리스 체크리스트
- 📈 **Data** (Data Analyst & BI Engineer): 데이터 분석, 대시보드, SQL, 스프레드시트, 지표 설계, 리포팅 자동화
- 🧩 **Product** (Product Manager): PRD, 로드맵, 고객 문제 정의, 기능 우선순위, 제품 실험, 출시 전략
- 🔁 **Automation** (Workflow Automation Engineer): MCP, API 연동, 업무 자동화, Zapier/Rube/Composio류 워크플로, 반복 작업 제거
- 📲 **Mobile** (Mobile App Engineer): Expo/React Native, Android, iOS, 모바일 UI, 앱 패키징, 스토어 출시 준비
