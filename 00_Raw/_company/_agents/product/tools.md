...
_Product 에이전트가 어떤 도구를 어디까지 자율적으로 쓸 수 있는지 정의합니다._
...
### `prd_writer`
문제 정의·사용자 스토리·수용 기준 기반 PRD 작성

- `enabled`: true
- `requires_credentials`: `config.md` 참조

### `roadmap_prioritizer`
RICE/ICE 기준 기능 우선순위 산정

- `enabled`: true
- `requires_credentials`: `config.md` 참조

### `experiment_planner`
제품 실험 가설·지표·기간·판정 기준 설계

- `enabled`: true
- `requires_credentials`: `config.md` 참조

### `launch_checklist`
출시 전 제품·마케팅·지원 체크리스트
...
- 모든 외부 행동은 `_agents/product/activity.log`에 한 줄 기록 (감사용).
