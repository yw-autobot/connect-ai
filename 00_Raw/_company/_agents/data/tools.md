...
_Data 에이전트가 어떤 도구를 어디까지 자율적으로 쓸 수 있는지 정의합니다._
...
### `report_generator`
분석 결과를 CEO/Business용 리포트로 변환

- `enabled`: true
- `requires_credentials`: `config.md` 참조

### `web_research_api`
Tavily/Web Research API를 사용한 읽기 전용 검색·추출
...
- 모든 외부 행동은 `_agents/data/activity.log`에 한 줄 기록 (감사용).
