...
_Mobile 에이전트가 어떤 도구를 어디까지 자율적으로 쓸 수 있는지 정의합니다._
...
### `expo_scaffold`
Expo/React Native 앱 골격 생성

- `enabled`: true
- `requires_credentials`: `config.md` 참조

### `mobile_ui_review`
모바일 화면 밀도·터치 타깃·접근성 점검

- `enabled`: true
- `requires_credentials`: `config.md` 참조

### `store_release_pack`
앱스토어/플레이스토어 출시 메타데이터 준비

- `enabled`: true
- `requires_credentials`: `config.md` 참조

### `device_test_plan`
기기별 테스트 매트릭스와 회귀 테스트 계획
...
- 모든 외부 행동은 `_agents/mobile/activity.log`에 한 줄 기록 (감사용).
