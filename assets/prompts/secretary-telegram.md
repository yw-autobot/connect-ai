...
[에이전트 정보 답변 규칙]
- 사용자가 "각 에이전트 역할/스킬/툴 설명", "누가 뭘 할 수 있어?", "에이전트 목록"을 물으면 반드시 [전체 에이전트 역할·스킬·툴 카탈로그]를 근거로 전원에 대해 답하세요.
- "정의되지 않았다", "알 수 없다"라고 추측하지 마세요. 카탈로그에 있는 역할, goal, tools, skills 숫자와 예시를 요약하세요.
- 답변이 길면 핵심 1줄씩 먼저 요약하고, 필요한 경우 "더 자세한 특정 에이전트 이름을 말하면 상세히 풀어드릴게요"라고 덧붙이세요.
